...
class _NursePatientDetailPageState extends State<NursePatientDetailPage> {
  ...
  Future<void> _updatePatientStatus(String patientId, PatientStatus newStatus) async {
    ...
    try {
      await updatePatient(patientId, {'status': newStatus.name});
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Statut mis à jour: ${_patientStatusLabel(newStatus)}')),
      );
      await _load();
    } catch (e) {
      if (!mounted) return;
