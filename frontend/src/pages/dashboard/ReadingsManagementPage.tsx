...
import { ReadingFilters, type ReadingFiltersState } from "@/components/dashboard/filters/ReadingFilters";
...
import type { FirestoreReading, FirestorePatient, FirestoreUser } from "@/types/firestore";

// Logging utility
const logError = (context: string, error: unknown, details?: Record<string, unknown>) => {
  console.error(`[ReadingsManagementPage] Error in ${context}:`, error, details);
};

const logWarning = (context: string, message: string, details?: Record<string, unknown>) => {
  console.warn(`[ReadingsManagementPage] Warning in ${context}:`, message, details);
};
...
export function ReadingsManagementPage() {
  ...
  const [editingReading, setEditingReading] = useState<FirestoreReading & { patientId?: string } | undefined>();
  ...
  useEffect(() => {
    logInfo("dataFetching", "Data fetching status", {
      ...
      readings: { loading: readingsLoading, count: readings.length, error: readingsError?.message },
    });
  }, [patientsLoading, usersLoading, readingsLoading, patients?.length, users?.length, readings.length, patientsError, usersError, readingsError]);
  ...
  useEffect(() => {
    const loadReadings = async () => {
      ...
      if (authLoading) {
        console.log("🔐 [ReadingsManagementPage] Auth still loading, skipping readings fetch");
        return;
      }
      ...
      console.group("🔐 [ReadingsManagementPage] Loading readings with permission check");
      ...
      if (!currentUserId) {
        console.warn("🔐 [ReadingsManagementPage] No user ID available - cannot load readings");
        setReadings([]);
      ...
      try {
        ...
        if (currentUser?.role === "admin") {
          ...
          console.log("🔐 [ReadingsManagementPage] Executing admin query: queryAllReadings");
          ...
          console.log("🔐 [ReadingsManagementPage] Admin query constraints:", constraints);
          
          try {
            allReadings = await queryAllReadings(constraints);
            console.log("🔐 [ReadingsManagementPage] Admin query successful, fetched", allReadings.length, "readings");
          } catch (queryError) {
            // If collection group query fails with permission error, fallback to per-patient fetching
            const errorObj = queryError instanceof Error ? queryError : new Error("Failed to query readings");
            const isPermissionError = errorObj.message.includes("permission") || 
                                      ...
                                      ('code' in errorObj && (errorObj as any).code === 'permission-denied');
            
            if (isPermissionError && currentUser?.role === "admin") {
              logInfo("loadReadings", "Using per-patient fallback (collection group query unavailable)", { filters });
              
              // Fallback: Fetch all active patients, then get readings from each
              const allPatients = await queryPatients([where("isActive", "==", true)]);
              logInfo("loadReadings", "Fallback: fetching readings per patient", { patientCount: allPatients.length });
              ...
              const readingsPromises = allPatients.map(async (patient) => {
                try {
                  ...
                  if (filters.readingType !== "all") {
                    patientConstraints.push(where("readingType", "==", filters.readingType));
                  }
                  if (filters.status !== "all") {
                  ...
                  if (filters.userId !== "all") {
                    patientConstraints.push(where("recordedById", "==", filters.userId));
                  }
                  
                  const patientReadings = await getReadings(patient.id, [...patientConstraints, limit(1000)]);
                  return patientReadings.map((reading) => ({
                    ...reading,
                    patientId: patient.id,
                  }));
                } catch (patientError) {
                  console.error("🔐 [ReadingsManagementPage] Error in fallback for patient:", patient.id, patientError);
                  return [];
                }
              });
              ...
              allReadings.sort((a, b) => {
                const dateA = a.date && typeof a.date.toDate === 'function' ? a.date.toDate() : (a.date instanceof Date ? a.date : new Date(0));
                const dateB = b.date && typeof b.date.toDate === 'function' ? b.date.toDate() : (b.date instanceof Date ? b.date : new Date(0));
                return dateB.getTime() - dateA.getTime();
              });
              
              logInfo("loadReadings", "Fallback successful", { count: allReadings.length });
          ...
          if (filters.patientId !== "all") {
            allReadings = allReadings.filter(r => r.patientId === filters.patientId);
            console.log("🔐 [ReadingsManagementPage] After patient filter:", allReadings.length, "readings");
          }
        } else if (currentUser) {
          // Non-admin users: fetch readings from assigned patients only
          console.log("🔐 [ReadingsManagementPage] Executing non-admin query: fetching from assigned patients");
          
          // Get assigned patients based on role
          let assignedPatients: FirestorePatient[] = [];
          if (currentUser.role === "doctor") {
            console.log("🔐 [ReadingsManagementPage] Doctor role: querying patients with doctorId =", currentUserId);
            assignedPatients = await queryPatients([
              where("doctorId", "==", currentUserId),
              where("isActive", "==", true),
            ]);
          } else if (currentUser.role === "nurse") {
            console.log("🔐 [ReadingsManagementPage] Nurse role: querying patients with nurseId =", currentUserId);
            assignedPatients = await queryPatients([
          ...
          console.log("🔐 [ReadingsManagementPage] Found", assignedPatients.length, "assigned patients");
          
          // Fetch readings from each assigned patient
          const readingsPromises = assignedPatients.map(async (patient) => {
            try {
              console.log("🔐 [ReadingsManagementPage] Fetching readings for patient:", patient.id, "patient name:", `${patient.firstName || ""} ${patient.lastName || ""}`);
              ...
              const patientReadings = await getReadings(patient.id, [...patientConstraints, limit(1000)]);
              console.log("🔐 [ReadingsManagementPage] Successfully fetched", patientReadings.length, "readings for patient:", patient.id);
              ...
              console.error("🔐 [ReadingsManagementPage] Error fetching readings for patient:", {
                patientId: patient.id,
                patientName: `${patient.firstName || ""} ${patient.lastName || ""}`,
                error: patientError,
                errorMessage: patientError instanceof Error ? patientError.message : String(patientError),
                errorStack: patientError instanceof Error ? patientError.stack : undefined,
                currentUser: {
                  id: currentUserId,
                  role: currentUser.role,
                  email: currentUser.email,
          ...
          const readingsArrays = await Promise.all(readingsPromises);
          allReadings = readingsArrays.flat();
          console.log("🔐 [ReadingsManagementPage] Total readings from assigned patients:", allReadings.length);
          ...
          // No current user
          console.warn("🔐 [ReadingsManagementPage] No current user - returning empty readings");
          allReadings = [];
        }
        
        logInfo("loadReadings", "Readings loaded successfully", { 
          count: allReadings.length,
          queryType: currentUser?.role === "admin" ? "queryAllReadings" : "getReadings from assigned patients",
        });
        ...
        if (isPermissionError) {
          console.warn("🔐 [ReadingsManagementPage] Permissions error detected:", errorObj.message);
          setReadings([]);
          setReadingsError(null);
    ...
    loadReadings();
  }, [filters.userId, filters.readingType, filters.status, filters.valueMin, filters.valueMax, filters.patientId, currentUser, authLoading, currentUserId]);

  // Filter and transform readings to match table format
  const filteredReadings = useMemo(() => {
    return readings
      .filter((reading) => {
        if (filters.patientId !== "all" && reading.patientId !== filters.patientId) return false;
        return true;
      })
      .map((reading) => {
        const patient = reading.patientId ? patientsMap.get(reading.patientId) : undefined;
        const user = reading.recordedById ? usersMap.get(reading.recordedById) : undefined;
        ...
        return {
          ...
          patient: {
            ...
            name: patient ? `${patient.firstName || ""} ${patient.lastName || ""}`.trim() : "Patient inconnu",
          ...
          recorded_by: {
            ...
            name: user ? `${user.firstName || ""} ${user.lastName || ""}`.trim() : reading.recordedByName || "Utilisateur inconnu",
            role: user?.role === "doctor" ? "Médecin" : user?.role === "nurse" ? "Infirmière" : "Utilisateur",
          },
  ...
  const handleFormSubmit = async (data: any) => {
    try {
      ...
      if (editingReading && editingReading.patientId) {
        ...
        logInfo("updateReading", "Updating reading", { readingId: editingReading.id, patientId: editingReading.patientId });
        await updateReading(
          editingReading.patientId,
          editingReading.id,
          {
            value: data.value,
            unit: data.unit,
        ...
        logInfo("updateReading", "Reading updated successfully", { readingId: editingReading.id });
        addNotification({
          type: "success",
        ...
        await createReading(
          ...
          currentUserId || "",
          currentUser ? `${currentUser.first_name || ""} ${currentUser.last_name || ""}`.trim() || undefined : undefined
        );
        logInfo("createReading", "Reading created successfully", { patientId: patientId });
        addNotification({
          type: "success",
          title: "Mesure créée",
      ...
      logError("formSubmit", error, { isEdit: !!editingReading, readingId: editingReading?.id, patientId: editingReading?.patientId || data.patient_id });
      addNotification({
        type: "error",
        title: "Erreur",
  ...
  const handleDeleteReading = async (reading: any) => {
    ...
    try {
      ...
      if (originalReading && originalReading.patientId) {
        logInfo("deleteReading", "Deleting reading", { readingId: originalReading.id, patientId: originalReading.patientId });
        await deleteReading(originalReading.patientId, originalReading.id);
        logInfo("deleteReading", "Reading deleted successfully", { readingId: originalReading.id });
        addNotification({
          type: "success",
  ...
  const handleBulkDelete = async () => {
    ...
    if (!confirm(`Êtes-vous sûr de vouloir supprimer ${selectedIds.length} mesure(s) ?`)) {
      return;
    }
    
    try {
      logInfo("bulkDelete", "Bulk deleting readings", { count: selectedIds.length, readingIds: selectedIds });
      await Promise.all(
        selectedIds.map(id => {
  ...
  if (readingsError) {
    ...
    const isPermissionsError = readingsError.message.includes("permissions") || 
                               readingsError.message.includes("insufficient");
    if (isPermissionsError) {
      logInfo("pageRender", "Permissions error - rendering with empty list", { errorMessage: readingsError.message });
      // Continue to render the page with empty readings list
    } else {
      logError("pageRender", "Rendering error state", { errorMessage: readingsError.message });
      return (
        <DashboardLayout>
          <ErrorMessage message={`Erreur lors du chargement des mesures: ${readingsError.message}`} />
        </DashboardLayout>
      );
    }
  }
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-foreground">Gestion des mesures</h1>
            <p className="text-muted-foreground mt-1">
              Consultez et gérez toutes les mesures de glycémie enregistrées dans le système
            </p>
          </div>
        ...
        {hasPermissionsError && (
          <Alert variant="default">
            ...
            <AlertDescription>
              Vous n'avez pas les permissions nécessaires pour afficher toutes les mesures. 
              La liste est vide. Veuillez contacter un administrateur si vous pensez que c'est une erreur.
            </AlertDescription>
          </Alert>
        )}
        ...
        <ReadingForm
          reading={editingReading ? {
            ...
            date: editingReading.date?.toDate().toISOString().split("T")[0] || new Date().toISOString().split("T")[0],
            time: editingReading.time || new Date().toTimeString().slice(0, 5),
