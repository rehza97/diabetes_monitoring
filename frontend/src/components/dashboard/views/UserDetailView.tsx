...
function useUserData(userId: string | undefined) {
  ...
  const { data: assignedPatients, loading: patientsLoading } = usePatients(
  ...
  const { data: loginHistory, loading: loginHistoryLoading } =
    useRealtimeAuditLogs(loginHistoryQuery);
...
function useUserReadings(
  userId: string | undefined,
  patients: FirestorePatient[] | null,
) {
    ...
    const loadReadings = async () => {
      ...
      try {
        ...
        if (currentUser?.role === "admin") {
          console.log(
            "Executing admin query: queryAllReadings with recordedById filter",
          );
          allReadings = await queryAllReadings([
          ...
          if (currentUser.role === "doctor") {
            console.log(
              "Querying assigned patients for doctor:",
              currentUserId,
            );
          ...
          console.log(
            "Total readings from assigned patients:",
            combinedReadings.length,
          );
        ...
        const enrichedReadings: EnrichedReading[] = allReadings.map(
          (reading) => ({
            ...reading,
        ...
        const error =
          err instanceof Error ? err : new Error("Failed to load readings");
        ...
        if (isPermissionError) {
          ...
          console.warn(
            "User doesn't have permission to view readings:",
            error.message,
          );
          setReadings([]);
        } else {
  ...
  useEffect(() => {
    ...
    const loadActivity = async () => {
      ...
      try {
        ...
        if (mounted) {
          setActivity(
            snapshot.docs.map((doc) => doc.data() as FirestoreAuditLog),
          );
        }
      } catch (error) {
...
function calculateActivityStats(
  ...
  const getReadingDate = (reading: EnrichedReading): Date | null => {
    ...
    if (
      reading.date &&
      typeof (reading.date as Timestamp).toDate === "function"
    ) {
  ...
  const avgDailyReadings =
    readingsLast30Days > 0 ? (readingsLast30Days / 30).toFixed(1) : "0";

  const loginCount30Days =
    loginHistory?.filter((l) => {
        ...
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="flex-1 grid gap-4 md:grid-cols-2">
            ...
            <InfoField
              ...
              value={
                user.lastLogin
                  ? formatDateTime(user.lastLogin.toDate().toISOString())
                  : "Jamais"
              }
            />
          </div>
...
export function UserDetailView() {
  ...
  const { readings, loading: readingsLoading } = useUserReadings(
    id,
    assignedPatients,
  );
  ...
  // Table columns
  const { patientsColumns, loginHistoryColumns, readingsColumns } =
    useTableColumns(navigate);
                      ...
                      >
                        <div className="flex-1">
                          <p className="text-sm font-medium">
                            {ACTION_LABELS[
                              act.action as keyof typeof ACTION_LABELS
                            ] || act.action}{" "}
                            {act.entityType}
                          </p>
                          <p className="text-xs text-muted-foreground mt-1">
                            {act.createdAt
                              ? formatDateTime(
                                  act.createdAt.toDate().toISOString(),
                                )
                              : "Date inconnue"}
                          </p>
                        </div>
                ...
                <Card>
                  ...
                  <CardContent>
                    <DataTable
                      ...
                      data={tableData.assignedPatients}
                    />
                  </CardContent>
                </Card>
              </TabsContent>
